...
export function DisplayStoreHomeModels({ units }: { units: Unit[] }) {
    ...
    const handlePull = async (model: string) => {
        const confirmed = window.confirm(`Voulez-vous vraiment exécuter le pull pour le modèle "${model}" ?`);
        if (!confirmed) return;
        try {
            setLoading((m) => ({ ...m, [model]: true }));
            alert(`Pull début: ${model}\nAttendez l'alerte de fin pour éviter les erreurs`);
    ...
    return (
        <div className="max-w-5xl mx-auto p-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
                <h2 className="text-2xl font-semibold text-gray-800">Store des modèles</h2>
                <div className="flex w-full sm:w-auto items-center gap-3">
                    <div className="relative flex-1 sm:w-72">
                        <input
                            className="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-700"
                ...
                {filtered.map((u) => {
                    ...
                    return (
                        <button
                            key={u.name}
                            type="button"
                            className="text-left bg-white border border-gray-200 rounded-xl p-4 shadow-sm hover:shadow-md transition h-full"
                            onClick={() => handlePull(tag)}
                            disabled={isLoading}
                        >
                            <div className="flex items-start justify-between">
                                <h2 className="mb-2 text-gray-900 text-xl font-medium">{u.name}</h2>
                                <span className="text-xs px-2 py-1 rounded-full bg-gray-100 text-gray-700 border border-gray-200">
                                    latest
                                </span>
                            </div>
                            <h3 className="text-sm text-gray-600 line-clamp-3">{u.desc}</h3>
            ...
            {filtered.length === 0 && (
                <p className="text-center text-gray-500 mt-10">Aucun modèle ne correspond à votre recherche.</p>
            )}
        </div>
    );
}
...
export function ListAvailableModel() {
    ...
    const filtered = useMemo(() => {
        const q = query.trim().toLowerCase();
        if (!q) return models;
        return models.filter((m) => m.name.toLowerCase().includes(q) || m.details.family?.toLowerCase().includes(q));
    }, [models, query]);

    return (
        <div className="max-w-4xl mx-auto p-6">
            <h2 className="text-2xl font-semibold mb-2 text-gray-800">Modèles installés</h2>
            <Link to="/ollama" className="inline-block text-gray-700 hover:underline mb-6">Retour</Link>
            <div className="mb-6">
                <input
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                    {filtered.map((model) => (
                        ...
                        >
                            ...
                            <p className="text-sm text-gray-600">Taille: {model.details.parameter_size}</p>
                            <p className="text-sm text-gray-500">{model.details.family}</p>
                        </div>
                    ))}
                    {filtered.length === 0 && (
                        <p className="col-span-full text-center text-gray-500">Aucun modèle trouvé.</p>
                    )}
                </div>
            )}
        </div>
